...
from generator import Conv_BatchNorm_ReLU
...
class PatchGAN(tf.keras.Model):
    def __init__(self, input_nc, output_nc, image_width, ndf=64):
        ...
        self.conv_2 = Conv_BatchNorm_ReLU(ndf * 4, used_in_encoder)
        self.conv_3 = Conv_BatchNorm_ReLU(ndf * 8, used_in_encoder, strides=(1, 1))
        self.conv_4 = Conv_BatchNorm_ReLU(1,       used_in_encoder, strides=(1, 1))
    ...
    def call(self, input_, output):
        ...
        """
        image_pairs = tf.concat([input_, output], -1)
        result = tf.nn.leaky_relu(self.conv_0(image_pairs), 0.2)
        for conv in (self.conv_1, self.conv_2, self.conv_3, self.conv_4):
            result = conv(result)
        return tf.math.reduce_mean(tf.math.sigmoid(result), axis=(1, 2, 3))
